...
struct AIClient {
    ...
    private static func request(payload: [String: Any], settings: [String: Any]) async throws -> [String: Any] {
        let instruction = Self.instruction + " 여러 결산을 입력받습니다. 최상위는 reports 배열이고 각 원소는 입력과 동일한 id 및 앞서 지정한 분석 JSON인 content를 포함해야 합니다. 모든 결산에 응답하세요. budget은 현재 설정된 월간 예산 참고값이며 과거 예산이나 연간 합계가 아닙니다."
        ...
        let reply = try await post(URL(string: endpoint)!, body: ["model": model,
            "messages": [["role": "system", "content": instruction], ["role": "user", "content": text]],
            "response_format": ["type": "json_object"], "max_tokens": 8192], key: key)
